...
var codesCanada = require('./codesCanada');
var codes = {};

var allUSZipCodes = _.values(codesUS.codes);
for(let item of allUSZipCodes) {
    item.city = item.city.trim().toLowerCase();
    item.state = item.state.trim().toLowerCase();
}

var allCAZipCodes = _.values(codesCanada.codes);
for(let item of allUSZipCodes) {
    item.city = item.city.trim().toLowerCase();
    item.state = item.state.trim().toLowerCase();
}
...
module.exports = {
    LookUpCityAndStateOfUS: function(city, stateRegion) {
        if(!_.isString(stateRegion) || !_.isString(city)) {
            return null;
        }

        return _.find(allUSZipCodes, function(item) {
            return (item.city === city.trim().toLowerCase()) &&
                (item.state === stateRegion.trim().toLowerCase());
        });
    },

    LookUpCityAndStateOfCA: function (city, stateName) {
        if(!_.isString(city) || !_.isString(stateName)) {
            return null;
        }

        return _.find(allUSZipCodes, function(item) {
            return (item.city === city.trim().toLowerCase()) &&
                (item.state === stateName.trim().toLowerCase());
        });
    },
    getZipCodesByStateOfUS: function (stateRegion) {
        if(!_.isString(stateRegion)) {
            return [];
        }
        return _.map(_.filter(allUSZipCodes, {state: stateRegion.trim().toLowerCase()}), 'zip');
    },
    getZipCodesByStateOfCA: function (stateName) {
        if(!_.isString(stateName)) {
            return [];
        }
        return _.map(_.filter(allUSZipCodes, {state: stateName}.trim().toLowerCase()), 'zip');
    },
};
